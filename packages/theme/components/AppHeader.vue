<template>
  <div>
    ...
    >
      ...
      <template #logo>
        <nuxt-link :to="localePath({ name: 'bg-home' })" class="sf-header__logo">
          <SfImage
            src="/icons/logo.svg"
...
import { useCart, useUser, cartGetters, useFacet, useCategory } from '@vue-storefront/bagisto';
import { computed, ref, watch, onBeforeUnmount, useRouter, useRoute } from '@nuxtjs/composition-api';
...
export default {
  ...
  setup(props, { root }) {
    ...
    const { result: searchListResult, search } = useFacet();

    const { categories: categoriesList, search: searchCategory } = useCategory();
    ...
    const handleSearch = debounce(async (paramValue) => {
      ...
      searchCategory({ apiType: 'categoryTree' });

      result.value = {
        products: searchListResult.value?.data?.items,
        categories: categoriesList?.value.slice(0, 5) || {}
      };
