          ...
          <div class="collected-product-list">
            <transition-group name="sf-fade" tag="div">
              <SfCollectedProduct
                ...
                :special-price="cartGetters.getItemPrice(itemProduct).special && $n(cartGetters.getItemPrice(itemProduct).special, 'currency')"
                :stock="99999"
              ...
              >
                ...
                <template #input>
                  <div class="sf-collected-product__quantity-wrapper">
                    <SfQuantitySelector
                      :disabled="loading"
                      :qty="cartGetters.getItemQty(itemProduct)"
                      class="sf-collected-product__quantity-selector"
                      @input="updateQuantity({ product: { id: itemProduct.id }, quantity: Number($event) })"
                    />
                  </div>
                </template>
              </SfCollectedProduct>
      ...
      <template #content-bottom>
        <transition name="sf-fade">
          <div v-if="totalItems">
            ...
            <SfProperty
              v-if="totals.shippingtotal"
              name="Delivery Charges"
              class="sf-property--full-width sf-property--large my-cart__total-price"
            >
              <template #value>
                <SfPrice
                  :regular="$n(totals.shippingtotal, 'currency')"
                />
              </template>
            </SfProperty>
            <SfProperty
            ...
            >
              <template #value>
                <SfPrice
                  :regular="`-${$n(totals.discounttotal, 'currency')}`"
                />
              </template>
            </SfProperty>
            <SfProperty
...
export default {
  ...
  setup() {
    ...
    const updateQuantity = debounce(async ({ product, quantity }) => {
      await updateItemQty({ product, quantity }).then(() => {
        sendNotification({
          key: 'item_updated',
          message: 'Cart\'s item updated successfully',
          type: 'success',
          title: 'Success!',
          icon: 'check'
        });
    ...
    const handleRemove = async (product) => {
      await removeItem(product).then(() => {
        sendNotification({
          key: 'item_deleted',
          message: 'Cart\'s item removed successfully',
          type: 'success',
          title: 'Success!',
          icon: 'check'
        });
