    ...
    <div class="main section">
      <div class="sidebar desktop-only">
        <LazyHydrate when-idle>
          <SfLoader
          :class="{ 'loading--categories': productListing }"
              ...
              >
                <template>
                  <SfList class="list">
                    ...
                    <SfDivider v-if="cat.children.length > 0" />
                    <SfListItem
                      class="list__item"
      ...
      <SfLoader :class="{ productListing }" :loading="productListing">
        <div class="products" v-if="!productListing">
          <transition-group
...
export default {
  ...
  setup(props, context) {
    ...
    const products = computed(() => facetGetters.getProducts(result?.value?.data, { master: true}));
    const pagination = computed(() => facetGetters.getPagination(result?.value?.data));
    const categoryTree = computed(() => facetGetters.getCategoryTree(result?.value?.data));

    const breadcrumbs = computed(() => facetGetters.getBreadcrumbs(result?.value?.data));
